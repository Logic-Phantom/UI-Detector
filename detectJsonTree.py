...
class DetectionJsonBuilder:
    ...
    def create_detection_group(self, detection, image_width, image_height):
        # 탐지된 객체에 대한 그룹 생성
        x1, y1, x2, y2 = detection.xyxy[0].tolist()
        conf = detection.conf[0]
        class_name = detection.names[int(detection.cls[0])]
    ...
    def detect_and_create_json(self, image_path):
        ...
        for result in results:
            ...
            for detection in result.boxes:
                detection_group = self.create_detection_group(detection, image_width, image_height)
...
def main():
    ...
    # 이미지 경로를 지정하여 실행
    image_path = "path_to_your_image.jpg"  # 실제 이미지 경로로 변경 필요
